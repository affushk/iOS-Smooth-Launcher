...
public class GlobalPanelService extends Service {
 ...
 private void showNotifications(){
  ...
  panel=root; WindowManager.LayoutParams p=new WindowManager.LayoutParams(-1,-1,WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_DIM_BEHIND,PixelFormat.TRANSLUCENT);p.gravity=Gravity.TOP;p.dimAmount=.12f;if(Build.VERSION.SDK_INT>=31){p.flags|=WindowManager.LayoutParams.FLAG_BLUR_BEHIND;p.setBlurBehindRadius(dp(22));}try{wm.addView(panel,p); panel.setTranslationY(-dp(90)); panel.setAlpha(0f); panel.animate().translationY(0).alpha(1f).setDuration(230).setInterpolator(new android.view.animation.DecelerateInterpolator()).start();}catch(Exception x){panel=null;}
 }
 ...
 private void hide(){if(panel!=null){final View old=panel;panel=null;old.animate().translationY(-dp(120)).alpha(0f).setDuration(190).setInterpolator(new android.view.animation.AccelerateInterpolator()).withEndAction(()->{try{wm.removeView(old);}catch(Exception ignored){}}).start();}}

...
public class GlobalPanelService extends Service {
 ...
 private void showNotifications(){
  ...
  for(IOSNotificationService.Item n:items){
   LinearLayout row=new LinearLayout(this); row.setGravity(Gravity.CENTER_VERTICAL); row.setPadding(dp(14),dp(12),dp(14),dp(12)); row.setBackground(round(ThemeEngine.glass(this,ThemeEngine.glassAlpha(this)),ThemeEngine.cardRadius(this)));
   ...
   row.setOnTouchListener(new View.OnTouchListener(){float x,y; public boolean onTouch(View v,MotionEvent e){if(e.getActionMasked()==0){x=e.getRawX();y=e.getRawY();return true;} if(e.getActionMasked()==2){float dx=e.getRawX()-x; v.setTranslationX(dx); v.setAlpha(Math.max(.25f,1f-Math.abs(dx)/dp(220f))); return true;} if(e.getActionMasked()==1){float dx=e.getRawX()-x;if(Math.abs(dx)>dp(72)){v.animate().translationX(dx>0?dp(420): -dp(420)).alpha(0f).setDuration(180).withEndAction(()->{IOSNotificationService.clearOne(n.key);((ViewGroup)v.getParent()).removeView(v);}).start();}else{v.animate().translationX(0).alpha(1).setDuration(150).start();}return true;}return true;}});
  ...
  panel=root; WindowManager.LayoutParams p=new WindowManager.LayoutParams(-1,-1,WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY,WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_DIM_BEHIND,PixelFormat.TRANSLUCENT);p.gravity=Gravity.TOP;p.dimAmount=.12f;if(Build.VERSION.SDK_INT>=31){p.flags|=WindowManager.LayoutParams.FLAG_BLUR_BEHIND;p.setBlurBehindRadius(dp(22));}try{wm.addView(panel,p); panel.setTranslationY(-dp(90)); panel.setAlpha(0f); panel.animate().translationY(0).alpha(1f).setDuration((long)(230*ThemeEngine.motion(this))).setInterpolator(new android.view.animation.DecelerateInterpolator()).start();}catch(Exception x){panel=null;}
 }
 ...
 private GradientDrawable round(int c,int r){GradientDrawable g=new GradientDrawable();g.setColor(c);g.setCornerRadius(dp(r));return g;} private int dp(int n){return Math.round(n*getResources().getDisplayMetrics().density);} private int dp(float n){return Math.round(n*getResources().getDisplayMetrics().density);}
 private void hide(){if(panel!=null){final View old=panel;panel=null;old.animate().translationY(-dp(120)).alpha(0f).setDuration((long)(190*ThemeEngine.motion(this))).setInterpolator(new android.view.animation.AccelerateInterpolator()).withEndAction(()->{try{wm.removeView(old);}catch(Exception ignored){}}).start();}}

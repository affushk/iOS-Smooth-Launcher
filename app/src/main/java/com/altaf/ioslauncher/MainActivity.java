...
public class MainActivity extends Activity {
    private LinearLayout body; private int accent=Color.rgb(83,145,255); private int pickMode=0; private int wallpaperTarget=3; private Uri wallpaperUri; private TextView saveButton;
    ...
    private void showHome(){base("Altaf Theme Studio"); EditText search=new EditText(this);search.setHint("Search themes");search.setHintTextColor(Color.LTGRAY);search.setTextColor(Color.WHITE);search.setSingleLine();search.setBackground(bg(Color.rgb(25,55,102),28));body.addView(search,new LinearLayout.LayoutParams(-1,dp(58)));
      ...
      TextView diag=card("⌕  Vivo Theme Diagnostic");diag.setOnClickListener(v->{pickMode=9;pick("*/*");});
    ...
    private void showSavedThemeActions(String n){String[] a={"Edit theme","Rename","Export .itz test","Delete"};new android.app.AlertDialog.Builder(this).setTitle(n).setItems(a,(d,w)->{if(w==0)showEditor(n);else if(w==1)renameTheme(n);else if(w==2)exportItzTest(n);else new android.app.AlertDialog.Builder(this).setTitle("Delete "+n+"?").setPositiveButton("Delete",(q,z)->{getSharedPreferences("themes",0).edit().remove("last_name").remove("saved").apply();showMyThemes();}).setNegativeButton("Cancel",null).show();}).show();}
    private void exportItzTest(String n){try{java.io.File out=new java.io.File(getExternalFilesDir(null),n.replaceAll("[^a-zA-Z0-9_-]","_")+".itz");java.util.zip.ZipOutputStream z=new java.util.zip.ZipOutputStream(new java.io.FileOutputStream(out));z.putNextEntry(new java.util.zip.ZipEntry("description.xml"));String xml="<?xml version=\"1.0\" encoding=\"UTF-8\"?><theme><name>"+n+"</name><designer>Altaf</designer><version>1.0</version></theme>";z.write(xml.getBytes("UTF-8"));z.closeEntry();z.putNextEntry(new java.util.zip.ZipEntry("preview/README.txt"));z.write("Altaf Theme Studio Vivo compatibility test package".getBytes("UTF-8"));z.closeEntry();z.close();Toast.makeText(this,"ITZ test exported: "+out.getName(),Toast.LENGTH_LONG).show();}catch(Exception e){Toast.makeText(this,"Export failed: "+e.getMessage(),Toast.LENGTH_LONG).show();}}
    ...
    private void showWallpaperTarget(){String[] x={"Home screen","Lock screen","Home + Lock"};new android.app.AlertDialog.Builder(this).setTitle("Apply wallpaper to").setItems(x,(d,w)->{wallpaperTarget=w==0?WallpaperManager.FLAG_SYSTEM:w==1?WallpaperManager.FLAG_LOCK:(WallpaperManager.FLAG_SYSTEM|WallpaperManager.FLAG_LOCK);pickMode=2;pick("image/*");}).show();}
    ...
    @Override protected void onActivityResult(int r,int c,Intent d){super.onActivityResult(r,c,d);if(r==22&&c==RESULT_OK&&d!=null){Uri u=d.getData();if(u!=null){try{getContentResolver().takePersistableUriPermission(u,Intent.FLAG_GRANT_READ_URI_PERMISSION);}catch(Exception ignored){}if(pickMode==2){wallpaperUri=u;getSharedPreferences("themes",0).edit().putString("wallpaper",u.toString()).apply();try(InputStream in=getContentResolver().openInputStream(u)){android.graphics.Bitmap bm=BitmapFactory.decodeStream(in);WallpaperManager.getInstance(this).setBitmap(bm,null,true,wallpaperTarget);Toast.makeText(this,wallpaperTarget==WallpaperManager.FLAG_SYSTEM?"Home wallpaper applied":wallpaperTarget==WallpaperManager.FLAG_LOCK?"Lock wallpaper applied":"Home + Lock wallpaper applied",Toast.LENGTH_LONG).show();}catch(Exception e){Toast.makeText(this,"Wallpaper saved; Android blocked apply",Toast.LENGTH_LONG).show();}}else if(pickMode==9){String report=u.getLastPathSegment()+" | "+u.toString();getSharedPreferences("themes",0).edit().putString("diag_file",report).apply();Toast.makeText(this,"Diagnostic captured: "+u.getLastPathSegment(),Toast.LENGTH_LONG).show();}else{getSharedPreferences("themes",0).edit().putString(pickMode==3?"font":"imported",u.toString()).apply();Toast.makeText(this,"Imported: "+u.getLastPathSegment(),Toast.LENGTH_LONG).show();}}}}
